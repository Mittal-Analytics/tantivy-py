...
class TestClass(object):
    ...
    def test_and_query(self, ram_index):
        ...
        query = index.parse_query(
            "title:men AND body:winter", ["title", "body"]
        )
    ...
    def test_and_query_parser_default_fields(self, ram_index):
        ...
        assert (
            repr(query)
            == """Query(TermQuery(Term(field=0, type=Str, "winter")))"""
        )
    ...
    def test_query_lenient(self, ram_index_numeric_fields):
        ...
        assert len(errors) == 0
        assert (
            repr(query) == """Query(TermQuery(Term(field=1, type=F64, 3.5)))"""
        )
    ...
    def test_doc_from_dict_json_validation(self):
        ...
        good = Document.from_dict(
            {"json": json.dumps(list_of_jsons[1])}, schema
        )
...
class TestSearcher(object):
    def test_searcher_repr(self, ram_index, ram_index_numeric_fields):
        assert (
            repr(ram_index.searcher())
            == "Searcher(num_docs=3, num_segments=1)"
        )
        assert (
...
class TestDocument(object):
    ...
    def test_document_eq(self):
        ...
        doc2 = tantivy.Document.from_dict(
            {"name": "Bill", "reference": [1, 2]}
        )
